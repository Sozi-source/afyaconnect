...
export default function ClientReviewsPage() {
  ...
  const fetchReviews = async () => {
    try {
      // ✅ Use the correct endpoint: getMyReviews() instead of getAll()
      const data = await apiClient.reviews.getMyReviews()
      // Ensure data is an array
      const userReviews = Array.isArray(data) ? data : []
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          {pendingConsultations.length > 0 && (
            <Link href="/client/dashboard/reviews/create" className="w-full sm:w-auto">
              <Button className="w-full sm:w-auto bg-teal-600 hover:bg-teal-700 text-white shadow-sm shadow-teal-200/50 relative">
                    ...
                    <CardBody className="p-5">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        <div className="flex-1">
                          ...
                          {review.comment && (
                            <p className="text-sm text-slate-600 mt-2 pl-[52px]">
                              "{review.comment}"
                            </p>
                          )}
                        </div>
