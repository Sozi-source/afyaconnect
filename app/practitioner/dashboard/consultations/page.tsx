...
export default function PractitionerConsultationsPage() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 overflow-x-hidden">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <Button
          ...
          variant="outline"
          size="sm"
          className="flex items-center gap-2 border-gray-200 w-fit"
        >
      ...
      {/* Stats - Fixed width grid */}
      <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-6 w-full">
        <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
          <p className="text-xs text-gray-500 mb-1">Total</p>
          <p className="text-lg sm:text-xl font-semibold text-gray-900 truncate">{totalCount}</p>
        </div>
        <div className="bg-blue-50 rounded-lg p-3 sm:p-4">
          <p className="text-xs text-blue-600 mb-1">Upcoming</p>
          <p className="text-lg sm:text-xl font-semibold text-blue-700 truncate">{upcomingCount}</p>
        </div>
        <div className="bg-green-50 rounded-lg p-3 sm:p-4">
          <p className="text-xs text-green-600 mb-1">Completed</p>
          <p className="text-lg sm:text-xl font-semibold text-green-700 truncate">{completedCount}</p>
        </div>
      </div>
      ...
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="bg-red-50 border border-red-200 rounded-lg p-3 mb-6 w-full"
          >
      ...
      <div className="space-y-3 mb-6 w-full">
        ...
        <div className="flex flex-wrap gap-1 p-1 bg-gray-50 rounded-lg w-fit">
          {(['upcoming', 'completed', 'cancelled', 'all'] as FilterType[]).map((tab) => (
            <button
              key={tab}
              onClick={() => setFilter(tab)}
              className={`px-3 py-1.5 text-xs sm:text-sm font-medium rounded-md transition whitespace-nowrap ${
                filter === tab
                    ...
                    <CardBody className="p-4">
                      ...
                      <div className="sm:hidden space-y-3">
                        <div className="flex items-start justify-between">
                          ...
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusBadge(consultation.status)} flex-shrink-0`}>
                      ...
                      <div className="hidden sm:flex sm:items-center sm:justify-between w-full">
                        ...
                        <div className="flex items-center gap-3 flex-1 justify-end">
                          ...
                          {consultation.status === 'booked' && (
                            <>
                              <button className="p-1.5 hover:bg-gray-50 rounded-lg transition flex-shrink-0" title="Video call">
                                <VideoCameraIcon className="h-4 w-4 text-gray-500" />
                              </button>
                              <button className="p-1.5 hover:bg-gray-50 rounded-lg transition flex-shrink-0" title="Phone call">
                      ...
                      {consultation.client_notes && (
                        <p className="mt-3 text-xs text-gray-500 bg-gray-50 p-2 rounded w-full">
                          📝 {consultation.client_notes}
                        </p>
                      )}
                      ...
                      {consultation.status === 'booked' && (
                        <div className="mt-3 flex flex-wrap gap-2 justify-end border-t border-gray-100 pt-3 w-full">
                          <button
