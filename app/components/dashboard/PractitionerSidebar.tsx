...
export function PractitionerSidebar({ isOpen, onClose }: PractitionerSidebarProps) {
  ...
  // Main navigation items for practitioners with readiness flag
  const mainNav = [
    { 
      name: 'Dashboard', 
    ...
    <AnimatePresence>
      {showComingSoon && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className="fixed top-24 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white px-4 py-2 rounded-lg text-sm z-50 whitespace-nowrap"
        >
          ...
          <div className="space-y-1">
            {mainNav.map((item) => (
              ...
              >
                ...
                {/* Description tooltip for ready items */}
                {item.isReady && (
                  <div className="absolute left-full ml-2 px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">
                    {item.description}
                  </div>
