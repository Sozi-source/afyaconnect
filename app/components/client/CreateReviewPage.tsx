...
export default function CreateReviewPage() {
  ...
  // CRITICAL: Mounting check MUST be first
  useEffect(() => {
    setIsMounted(true)
  }, [])

  // Auth loading check - MUST be first return
  ...
  // Data fetching - runs only after auth is confirmed
  useEffect(() => {
    if (!consultationId) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 px-4 py-4 sm:py-6">
      <div className="flex items-center gap-3">
        <Link href={`/client/dashboard/consultations/${consultationId}`} className="p-2 hover:bg-neutral-100 rounded-lg">
        ...
        <CardBody className="p-4 sm:p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="bg-neutral-50 rounded-xl p-4">
              ...
              <p className="font-medium text-neutral-900">Dr. {consultation.practitioner_name || 'Unknown'}</p>
            ...
            <div>
              ...
              <textarea id="comment" rows={5} value={comment} onChange={(e) => setComment(e.target.value)}
                ...
                className="w-full px-4 py-3 border border-neutral-200 rounded-xl focus:ring-2 focus:ring-primary-500 bg-white resize-none"
                maxLength={500}
              />
              <p className="mt-1 text-xs text-neutral-400 text-right">{comment.length}/500</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:justify-end gap-3 pt-4 border-t border-neutral-200">
              <Link href={`/client/dashboard/consultations/${consultationId}`} className="w-full sm:w-auto">
                <Button variant="outline" type="button" fullWidth className="sm:w-auto">Cancel</Button>
              </Link>
              <Button type="submit" disabled={submitting || rating === 0} isLoading={submitting} fullWidth
